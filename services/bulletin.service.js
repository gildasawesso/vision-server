...
function getExaminationSession(examination, schoolYear) {
  for (const session of schoolYear.sessions) {
    if (
      moment(examination.examinationDate).isBetween(moment(session.startDate), moment(session.endDate), 'day', '[]')
    ) {
      return session;
    }
  }
...
async function buildSessionNotesTree(schoolId, schoolYear) {
  ...
  for (const examination of examinations) {
    ...
    const currentSubjectStudents = currentClassroomSubjects[examination.subject._id].students;
    ...
    examination.marks.forEach(mark => {
      ...
      if (mark.mark != null) {
        ...
        currentSubjectExaminationTypes[mark.student._id].mean =
          sum / currentSubjectExaminationTypes[mark.student._id].notes.length;
        ...
        if (!currentSubjectStudents[mark.student._id].examinationTypes.includes(examination.type._id)) {
          ...
          currentSubjectStudents[mark.student._id].mean =
            allSubjectsNotes / currentSubjectStudents[mark.student._id].notes.length;
        }
        ...
        currentClassroomStudents[mark.student._id].subjects[examination.subject._id].note =
          currentSubjectStudents[mark.student._id].mean;
        currentClassroomStudents[mark.student._id].subjects[examination.subject._id].coef =
          examination.subject.coefficient;

        currentClassroomStudents[mark.student._id].subjects[examination.subject._id].totalWithCoef =
          currentSubjectStudents[mark.student._id].mean * examination.subject.coefficient;
        currentClassroomStudents[mark.student._id].subjects[examination.subject._id].totalWithCoef =
          currentSubjectStudents[mark.student._id].mean * examination.subject.coefficient;
